...
- 每项工作都能在自己的worktree中实施、测试、提交和记录；
...
主工作树保留主线现状，用来核对公共起点、依次接收结果和做整体验证。每个并行任务在独立worktree中开展，不让主工作树同时带着其中一项未完成的差异。

分支名称可以由小写ACL编号和简短主题组成，例如`acl-v0.11-001-conversation-favorites`。分支跟踪的是工作而不是Agent身份，不要使用模型名或Agent名命名。

建立worktree前，先取得准备作为起点的完整提交，再从这个提交创建短期分支和目录：

```bash
task_start_commit="$(git rev-parse HEAD)"
git worktree add \
  -b acl-v0.11-001-conversation-favorites \
  ../hive-os-worktrees/acl-v0.11-001-conversation-favorites \
  "$task_start_commit"
```
...
```bash
git merge --ff-only acl-v0.11-001-conversation-favorites
```
...
一项分支进入主线，不表示它的ACL已经结束。实际修改、验证、失败和跳过项、剩余问题以及Director的结论确认仍要完整保留。后续章节会分别说明实施与发布、Loop结束和版本形成。
