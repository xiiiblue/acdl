...
|---|---|---|---|
|DFL(Developer Feedback Loop)|Director|真实环境中的完整任务和使用感受|产品在实际工作中是否真的好用|
|ERL(Engineering Review Loop)|高阶Agent|固定提交或Git标签中的设计、代码、测试和发布材料|现有结果是否隐藏工程风险|

DFL不是人工执行一遍自动化测试，ERL也不是让原来的Agent再解释一次自己的代码。两者都要从实施者以外的视角寻找ACL(Agentic Coding Loop)和自动化验证不容易发现的问题。

它们也不是ACL之后必须依次完成的两个阶段。Director可以单独对运行已久的功能开展DFL，高阶Agent也可以单独评审某个关键模块或正式版本。是否启动、何时启动、检查多深，由当前问题和风险决定。
...
普通DFL把启动方案、当前进展、真实操作、发现和结论保存在同一份实施记录中。高风险专项需要单独确认操作范围时，再增加独立计划。正式ERL始终同时保留独立评审计划和评审记录：计划固定准备怎样检查，记录保存实际检查和发现。
...
真实使用中经常会发现很小的问题。ACDL允许在DFL中处理不改变Accepted设计的最小修复，例如补上已经明确要求的等待反馈、改正不影响流程含义的文字，或者恢复一个已经确认却没有实现的交互状态。

这种现场修复仍然需要定点测试、必要的环境更新和再次使用，实际修改和结果继续写进DFL记录。是否适合留在当前DFL，可以用三个问题判断：

1.产品和设计是否已经明确说明正确行为？
2.修改是否局部、低风险，并且可以在本轮内验证？
3.修改是否不会改变流程、状态、权限、安全、数据边界或正式范围？

只要其中一个问题无法肯定回答，就不要为了让DFL一次结束而顺手扩大修改。跨模块、风险较大或需要独立验证的缺陷进入ISSUE，随后建立ACL处理；希望新增收藏文件夹等新行为则进入REQ，重新完成产品与架构设计。

## ERL(Engineering Review Loop)：让高阶Agent独立检查固定结果

ERL由高阶Agent主导。高阶Agent通常使用推理能力更强的模型，从评审者而非实现者的角度，独立检查一个不会在评审中继续变化的提交或Git标签。Director确定为什么评审、评审什么以及哪些风险值得优先关注，高阶Agent负责展开调查、提出候选发现并寻找证据。

“高阶”不等于只把模型名字换得更昂贵。一次ERL至少需要三种变化：

- 使用独立上下文重新理解问题，不继承原实现过程中的全部判断；
- 读取Accepted设计、完整差异和验证结果，而不是只听原Agent概述；
...
每轮正式ERL都有独立评审计划和评审记录。计划与结果分开，是为了防止评审结束后再根据发现悄悄改变原来的范围和判定标准。

开始前，Director与高阶Agent固定：
...
## 高阶Agent先提出可能存在的问题，再用证据确认

高阶Agent和辅助工具可能给出大量看似合理的风险，其中一部分来自误读、重复发现或不适用于当前设计的通用建议。ERL的价值不在于生成更长的问题列表，而在于把真正成立的问题找出来。

每项候选发现都要回到Accepted设计、代码、测试或运行事实复核：
...
只有“这里可能有并发问题”还不够。高阶Agent需要进一步说明哪些操作可能同时发生、当前代码怎样处理、会出现什么错误结果，以及现有测试为什么没有发现它。无法取得证据的推测可以保留为未确认候选，但不能登记成已经确认的问题。
        ...
        →改变产品行为或正式边界
          →登记REQ
            →重新完成产品与架构设计
              →建立ACL实施
        →环境自身问题
          →更新环境记录，并按需要建立ISSUE或维护ACL
```

例如，DFL发现刷新后收藏状态丢失，而Accepted设计已经明确要求状态持久保存，这是实现缺陷，可以进入ISSUE和ACL。DFL发现收藏超过十个后需要文件夹，则是在增加新对象和新流程，需要登记REQ。ERL发现员工可以读取他人的收藏数据，是实现或安全缺陷；如果修复需要重新定义跨员工共享行为，则还要回到REQ和相应层级设计。
