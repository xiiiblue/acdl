...
|当前设计|`docs/02-设计/README.md`及系统、能力、横切设计目录|Accepted产品与架构结论、ADR、索引和稳定设计要求正文|
...
|能力设计|`CAP-三位编号-标题.md`；专项设计可用`CAP-三位编号-两位子编号`|全局连续，不复用|
...
Loop在启动内容已经准备好、但还没有第一次真实执行时可以处于`Ready`；开始执行后进入`In Progress`。现行主要变化是：
...
`Review`表示执行结束，证据和剩余问题已经整理，等待结论确认或合入；它也可以因为发现缺口回到`In Progress`。`Completed`和`Withdrawn`是最终状态。ACL进入`Completed`还要求已经合入目标分支，并记录实际合入提交。结果另行记录，不能从`Completed`推断“没有遗留问题”。
...
当前`loop:new`只创建ACL。ACL身份与版本分开，计划版本尚未确定时填写`未确定`：
...
默认创建为`In Progress`。启动内容已经准备好、尚未发生第一次真实执行时，增加`--ready`创建为`Ready`。命令会在单一主记录开头生成启动方案；`--mainline`和`--with-plan`已经移除。来源精确填写一个`ISSUE-六位编号`时，工具还会把新ACL写回ISSUE台账的处理关系。

DFL和ERL当前不能自动创建文件，需要在各自登记表取得下一编号，并按相应模板建立主记录。是否另建独立计划，取决于执行前是否需要单独批准高风险、恢复或评审边界。

ACL进入`Review`使用专门入口，DFL和ERL使用通用状态命令。状态修改都先预览，再正式执行：
...
`loop:review`会检查ACL候选和最近一次成功的`pnpm verify`证据是否匹配。进入`Completed`前需要Director已经确认；ACL还必须已经合入目标分支，并在主记录填写真实`主线合入提交`。`--confirm`只记录确认了什么，不能代替确认本身。
