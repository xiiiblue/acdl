...
设计类别按事实职责划分，不要求每个REQ新建一组文档。先找到真正负责当前事实的设计，再检查相关共同规则是否需要同步。旧`PLAT / DOMAIN / MODULE`只用于历史追溯。

ADR解释为什么选择某个重要方案，最终决定仍要进入受影响的系统、能力或横切设计。推翻Accepted ADR时建立新的ADR，并让旧记录进入`Superseded`。
